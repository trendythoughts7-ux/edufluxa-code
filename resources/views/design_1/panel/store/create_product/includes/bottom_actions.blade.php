<div class="position-fixed position-bottom-0 position-left-0 position-right-0 z-index-3 bg-white soft-shadow-2">
    <div class="container d-flex flex-column flex-lg-row align-items-lg-center justify-content-lg-between py-16">
        ...
        <div class="d-flex align-items-center">
            {{-- Previous --}}
            <a href="{{ (!empty($product) and $currentStep > 1) ? ("/panel/store/products/{$product->id}/step/" . ($currentStep - 1)) : '#!' }}" class="d-flex-center size-48 rounded-circle bg-gray-100">
                @svg("iconsax-lin-arrow-left", ['height' => 16, 'width' => 16, 'class' => (!empty($product) and $currentStep > 1) ? 'text-primary' : 'text-gray-500'])
            </a>
