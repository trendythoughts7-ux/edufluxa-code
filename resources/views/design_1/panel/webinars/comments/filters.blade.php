<form action="/panel/courses/comments" method="get" class="px-16">
    <div class="row mt-24">
            ...
            <div class="form-group ">
                ...
                <select name="course_id" class="form-control select2"  data-placeholder="{{ trans('update.search_courses') }}">
                    ...
                    @foreach($myCourses as $myCourse)
                        <option value="{{ $myCourse->id }}" {{ (!empty($selectedCourse) and $selectedCourse->id == $myCourse->id) ? 'selected' : '' }}>{{ $myCourse->title }}</option>
                    @endforeach
