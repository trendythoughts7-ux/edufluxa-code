...
@section('content')
    ...
    @if(!empty($comments) and !$comments->isEmpty())
        <div class="bg-white pt-16 rounded-24 ">
            ...
            {{-- Filters --}}
            @include('design_1.panel.jobs.my_comments.filters')

            {{-- List Table --}}
            <div id="tableListContainer" class="table-responsive-lg" data-view-data-path="/panel/jobs/my-comments">
                <table class="table panel-table">
                    ...
                    @foreach($comments as $commentRow)
                        @include('design_1.panel.jobs.my_comments.table_items', ['comment' => $commentRow])
                    @endforeach
                    </tbody>
                </table>
