...
<div class="bg-white rounded-16 p-16 mt-32">
    ...
    <div class="row">
        <div class="col-lg-6 mt-20">
            @include('design_1.panel.jobs.create.includes.accordions.prerequisite')
        </div>

        <div class="col-lg-6 mt-36">
            @if(!empty($job->prerequisites) and count($job->prerequisites))
                <div class="p-16 rounded-16 border-gray-200">
                    ...
                    <ul class="draggable-content-lists prerequisites-draggable-lists" data-path="" data-drag-class="prerequisites-draggable-lists">
                        @foreach($job->prerequisites as $prerequisiteInfo)
                            @include('design_1.panel.jobs.create.includes.accordions.prerequisite',['prerequisite' => $prerequisiteInfo])
                        @endforeach
                    </ul>
                </div>
            @else
    ...
    <div class="row">
        <div class="col-lg-6 mt-20">
            @include('design_1.panel.jobs.create.includes.accordions.related_courses')
        </div>

        <div class="col-lg-6 mt-36">
            @if(!empty($job->relatedCourses) and count($job->relatedCourses))
                <div class="p-16 rounded-16 border-gray-200">
                    ...
                    <ul class="draggable-content-lists related_courses-draggable-lists" data-path="" data-drag-class="related_courses-draggable-lists">
                        @foreach($job->relatedCourses as $relatedCourseInfo)
                            @include('design_1.panel.jobs.create.includes.accordions.related_courses',['relatedCourse' => $relatedCourseInfo])
                        @endforeach
                    </ul>
                </div>
            @else
