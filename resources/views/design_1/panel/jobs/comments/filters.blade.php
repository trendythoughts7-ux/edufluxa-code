<form action="/panel/jobs/comments" method="get" class="px-16">
    <div class="row mt-24">
        ...
        <div class="col-6 col-md-3">
            <div class="form-group ">
                <label class="form-group-label">{{ trans('update.job') }}</label>

                <select name="job_id" class="form-control select2"  data-placeholder="{{ trans('update.select_a_job') }}">
                    <option value="">{{ trans('public.all') }}</option>

                    @foreach($myJobs as $myJob)
                        <option value="{{ $myJob->id }}" {{ (!empty($selectedJob) and $selectedJob->id == $myJob->id) ? 'selected' : '' }}>{{ $myJob->title }}</option>
                    @endforeach
