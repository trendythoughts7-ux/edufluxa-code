    ...
    @if(!empty($purchasedTickets) and !$purchasedTickets->isEmpty())
        <div class="bg-white pt-16 rounded-24 mt-20">
            <div class="d-flex align-items-center justify-content-between pb-16 px-16 border-bottom-gray-100">
                <div class="">
                    <h3 class="font-16">{{ trans('update.my_tickets') }}</h3>
                    <p class="font-14 text-gray-500 mt-4">{{ trans('update.manage_your_event_tickets_and_details') }}</p>
                </div>
            </div>

            {{-- Filters --}}
            @include('design_1.panel.events.my_purchases.purchased_tickets.filters')

            {{-- List Table --}}
            <div id="tableListContainer" class="table-responsive-lg" data-view-data-path="/panel/events/my-purchases/{{ $event->id }}/tickets">
                <table class="table panel-table">
                    <thead>
                    <tr>
                        <th class="text-left">{{ trans('update.ticket') }}</th>
                    ...
                    @foreach($purchasedTickets as $saleRow)
                        @include('design_1.panel.events.my_purchases.purchased_tickets.table_items', ['purchasedTicket' => $saleRow])
                    @endforeach
                    </tbody>
                </table>
