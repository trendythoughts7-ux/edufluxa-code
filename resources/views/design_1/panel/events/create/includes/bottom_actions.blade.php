<div class="position-fixed position-bottom-0 position-left-0 position-right-0 z-index-3 bg-white soft-shadow-2">
    <div class="container d-flex flex-column flex-lg-row align-items-lg-center justify-content-lg-between py-16">
        ...
        <div class="d-flex align-items-center">
            {{-- Previous --}}
            <a href="{{ (!empty($event) and $currentStep > 1) ? ("/panel/events/{$event->id}/step/" . ($currentStep - 1)) : '#!' }}" class="d-flex-center size-48 rounded-circle bg-gray-100">
                @svg("iconsax-lin-arrow-left", ['height' => 16, 'width' => 16, 'class' => (!empty($event) and $currentStep > 1) ? 'text-primary' : 'text-gray-500'])
            </a>
        ...
        <div class="d-flex align-items-center mt-20 mt-lg-0 gap-8">
            ...
            @if(!empty($event) and $event->creator_id == $authUser->id)
                @include('design_1.panel.includes.content_delete_btn', [
                    'deleteContentUrl' => "/panel/events/{$event->id}/delete?redirect_to=/panel/events",
            ...
            {{-- Send for Review --}}
            <button type="button" id="sendForReview" class="btn btn-lg btn-primary ml-16">{{ !empty(getGeneralOptionsSettings('direct_publication_of_events')) ? trans('update.publish') : trans('public.send_for_review') }}</button>
