...
@section('content')
    <form method="post" action="/panel/events/{{ !empty($event) ? $event->id .'/update' : 'store' }}" id="webinarForm" enctype="multipart/form-data">
        {{ csrf_field() }}
        ...
        <div class="container mt-80 pb-100">
            {{-- Progress --}}
            @include('design_1.panel.events.create.includes.progress')

            {{-- Steps Inputs --}}
            @include("design_1.panel.events.create.steps.step_{$currentStep}")
        </div>


        {{-- Bottom Actions --}}
        @include('design_1.panel.events.create.includes.bottom_actions')
