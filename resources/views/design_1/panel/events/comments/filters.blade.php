<form action="/panel/events/comments" method="get" class="px-16">
    <div class="row mt-24">
        ...
        <div class="col-6 col-md-3">
            <div class="form-group ">
                <label class="form-group-label">{{ trans('update.event') }}</label>

                <select name="event_id" class="form-control select2"  data-placeholder="{{ trans('update.select_a_event') }}">
                    <option value="">{{ trans('public.all') }}</option>

                    @foreach($myEvents as $myEvent)
                        <option value="{{ $myEvent->id }}" {{ (!empty($selectedEvent) and $selectedEvent->id == $myEvent->id) ? 'selected' : '' }}>{{ $myEvent->title }}</option>
                    @endforeach
