@php
    $activeAgora = !empty(getFeaturesSettings('agora_for_events'));
    $selectedSessionApi = "external";

    if ($activeAgora and !empty($event->session) and $event->session->session_api == "agora") {
        $selectedSessionApi = "agora";
    }
@endphp
...
<div class="js-events-create-session-form mt-28" data-action="/panel/events/{{ $event->id }}/create-session">

    @if($activeAgora)
        <div class="d-flex align-items-center gap-5 p-4 border-gray-300 rounded-12 mt-8">
            @foreach(['external', 'agora'] as $sessionType)
                <div class="custom-input-button custom-input-button-none-border-and-active-bg  position-relative flex-1">
                    <input type="radio" class="" name="session_type" id="session_type_{{ $sessionType }}" value="{{ $sessionType }}" {{ ($sessionType == $selectedSessionApi) ? 'checked' : '' }}>
    ...
    @if($activeAgora)
        <div class="js-agora-session-fields mt-32 mb-40 {{ ($selectedSessionApi == "agora") ? '' : 'd-none' }}">
    ...
    <div class="js-external-session-fields mt-28 {{ ($selectedSessionApi == "external") ? '' : 'd-none' }}">

        <div class="form-group">
            <label class="form-group-label">{{ trans('update.session_url') }}</label>
            <input type="text" name="url" class="js-ajax-url form-control" value="{{ (!empty($event->session) and !empty($event->session->link)) ? $event->session->link : '' }}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="password" class="js-ajax-password form-control" value="{{ (!empty($event->session) and !empty($event->session->api_secret)) ? $event->session->api_secret : '' }}"/>
            <div class="invalid-feedback"></div>
        </div>
