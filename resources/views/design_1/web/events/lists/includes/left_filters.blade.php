<div class="position-relative events-lists-filters">
    ...
    <div id="leftFiltersAccordion" class="position-relative card-before-line card-before-line__4-12 bg-white py-16 rounded-24 z-index-2">
        ...
        @if(!empty($categoriesLists) and count($categoriesLists))
            @foreach($categoriesLists as $category)
                @if(!empty($category->subCategories) and count($category->subCategories))
                    <div class="accordion border-bottom-gray-100 mt-16 px-16 pb-16">
                        <div class="accordion__title d-flex align-items-center justify-content-between">
                            <div class="font-14 font-weight-bold text-dark cursor-pointer" href="#leftFiltersCategory{{ $category->id }}" data-parent="#leftFiltersAccordion" role="button" data-toggle="collapse">
                                {{ $category->title }}
                            </div>

                            <span class="collapse-arrow-icon d-flex cursor-pointer" href="#leftFiltersCategory{{ $category->id }}" data-parent="#leftFiltersAccordion" role="button" data-toggle="collapse">
                            <x-iconsax-lin-arrow-up-1 class="icons text-gray-500" width="16"/>
                        </span>
                        </div>

                        <div id="leftFiltersCategory{{ $category->id }}" class="accordion__collapse pt-0 mt-0 border-0 {{ $loop->first ? 'show' : '' }}" role="tabpanel">
                            <div class="pl-8">
                ...
                @else
                    <div class="js-get-view-data-by-tab mt-16 px-16 cursor-pointer font-weight-bold {{ (request()->get("category_id") == $category->id) ? 'active' :'' }}" data-filter-name="category_id" data-filter-value="{{ $category->id }}" data-container-id="listsContainer">{{ $category->title }}</div>
                @endif
            @endforeach
        @endif
    </div>
    ...
    <div id="leftFiltersAccordion" class="position-relative bg-white py-16 rounded-24 z-index-2">
        ...
        <div class="accordion card-before-line card-before-line__4-12 pb-16 px-16 border-bottom-gray-100">
            ...
            <div id="leftFiltersTypes" class="accordion__collapse show pt-0 mt-0 border-0" role="tabpanel">
                @foreach(['in_person', 'online'] as $typeOption)
                    <div class="custom-control custom-checkbox {{ $loop->first ? 'mt-16' : 'mt-12' }}">
                        <input type="checkbox" name="types[]" value="{{ $typeOption }}" id="filter_type_{{ $typeOption }}" class="custom-control-input">
        ...
        <div class="accordion card-before-line card-before-line__4-12 pb-16 px-16 border-bottom-gray-100">
            ...
            <div id="leftFiltersMoreOptions" class="accordion__collapse show pt-0 mt-0 border-0" role="tabpanel">
                @foreach(['supported_events', 'certificate_included', 'point_events'] as $moreOption)
                    <div class="custom-control custom-checkbox {{ $loop->first ? 'mt-16' : 'mt-12' }}">
