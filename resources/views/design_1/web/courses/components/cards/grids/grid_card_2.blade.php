<a href="{{ $course->getUrl() }}" class="text-decoration-none d-block">
    <div class="course-grid-card-2 position-relative bg-gray-200 rounded-16">
        <img src="{{ $course->getImage() }}" class="img-cover rounded-16" alt="{{ $course->title }}">
        ...
        <div class="course-grid-card-2__body d-flex flex-column p-12 rounded-8">
            <div class="d-flex flex-column w-100">
                <h3 class="course-title font-14 font-weight-bold text-dark">{{ clean($course->title,'title') }}</h3>
                    ...
                    <div class="d-flex flex-column ml-4">
                        ...
                        @if(!empty($course->category))
                            <div class="d-inline-flex align-items-center gap-4 mt-2 font-12 text-gray-500">
                                ...
                                <span class="font-14 text-gray-500 text-ellipsis">{{ $course->category->title }}</span>
                            </div>
                        @endif
                    </div>
                </div>
