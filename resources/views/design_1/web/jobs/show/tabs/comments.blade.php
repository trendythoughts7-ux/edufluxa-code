...
<div class="bg-white p-16 rounded-24">
    ...
    @include('design_1.web.components.comments.submit_form', [
        'commentForItemId' => $job->id,
        'commentForItemName' => "job_id",
    ])

    {{-- Comments Lists --}}
    @if(!empty($jobComments) and count($jobComments['comments']))
        <div class="js-course-comments-container">
            @include('design_1.web.components.comments.all_cards', [
                'comments' => $jobComments['comments'],
                'commentForItemId' => $job->id,
                'commentForItemName' => "job_id",
            ])
        </div>

        @if(!empty($jobComments['has_more']))
            <div class="d-flex-center mt-16">
                <button type="button" class="js-comments-load-more-btn d-flex-center py-16 px-24 rounded-12 border-dashed border-gray-300 text-gray-500 bg-white bg-hover-gray-100 cursor-pointer" data-path="/comments/lists/job/{{ $job->id }}">
