...
<form class="js-set-tracking-code-form" action="{{ getAdminPanelUrl("/store/orders/{$order->sale_id}/productOrder/{$order->id}/setTrackingCode") }}" method="post">
    {{ csrf_field() }}

    <div class="form-group mt-24">
        <label class="form-group-label">{{ trans('update.tracking_code') }}</label>
        <input type="text" name="tracking_code" class="js-ajax-tracking_code form-control" value="{{ !empty($order->tracking_code) ? $order->tracking_code : '' }}" placeholder=""/>
