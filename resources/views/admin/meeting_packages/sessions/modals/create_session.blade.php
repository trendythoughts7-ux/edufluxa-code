@php
    ...
    if ($activeAgora and !empty($session) and $session->session_api == "agora") {
...
<div class="js-meeting-package-create-session-form mt-28" data-action="{{ getAdminPanelUrl("/meeting-packages/sold/{$meetingPackageSold->id}/sessions/{$session->id}/set-api") }}">
    ...
    <div class="js-external-session-fields mt-28 {{ ($selectedSessionApi == "external") ? '' : 'd-none' }}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="url" class="js-ajax-url form-control" value="{{ (!empty($session) and !empty($session->link)) ? $session->link : '' }}">
        ...
        <div class="form-group">
            ...
            <input type="text" name="password" class="js-ajax-password form-control" value="{{ (!empty($session) and !empty($session->api_secret)) ? $session->api_secret : '' }}"/>
            <div class="invalid-feedback"></div>
        </div>
