...
@section('content')
    <form action="{{ getAdminPanelUrl("/themes/headers/{$headerItem->id}/update") }}" method="post" enctype="multipart/form-data" class="pb-64">
        {{ csrf_field() }}

        <div class="bg-white p-16 rounded-24 ">
            <div class="d-flex align-items-center justify-content-between pb-16 border-bottom-gray-100 pr-8">
                <div class="">
                    <h2 class="font-16 text-black">{{ $headerItem->title }}</h2>
                </div>

            </div>

            @include("admin.theme.headers.components.{$headerItem->component_name}.index")
        ...
        <div class="d-flex align-items-center justify-content-between bg-white px-16 py-16 mt-16 rounded-24">
            <div class="d-flex align-items-center">
                ...
                <div class="ml-8">
                    <h6 class="font-14 text-black m-0">{{ trans('update.information') }}</h6>
                    <p class="font-12 text-gray-500 mb-0">{{ trans('update.information_hint') }}</p>
                </div>
            </div>
