...
<div class="row">
    <div class="col-12 col-md-6">
        ...
        <div class="js-apply-cashback-per-item form-group {{ (empty($rule) or $rule->amount_type == 'fixed_amount') ? '' : 'd-none' }}">
            ...
            <p class="font-12 text-gray-500 mt-1">{{ trans('update.apply_cashback_per_item_hint') }}</p>
        </div>
        ...
        <div class="js-max-amount-field form-group {{ (!empty($rule) and $rule->amount_type == 'percent') ? '' : 'd-none' }}">
            ...
            <div class="text-gray-500 text-small mt-1">{{ trans('update.cashback_max_amount_hint') }}</div>
            @error('max_amount')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
        ...
        <div class="form-group">
            ...
            <div class="text-gray-500 text-small mt-1">{{ trans('update.cashback_min_amount_hint') }}</div>
            @error('min_amount')
            <div class="invalid-feedback">
                {{ $message }}
            </div>
