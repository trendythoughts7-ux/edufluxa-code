@foreach(\App\Models\WebinarExtraDescription::$types as $extraDescriptionType)
    <section class="mt-30">
        <div class="d-flex justify-content-between align-items-center">
            <h2 class="section-title after-line">{{ trans('update.job_'.$extraDescriptionType) }}</h2>

            <button type="button"
                    class="js-job-contents-action-btn btn btn-primary btn-sm mt-3"
                    data-path="{{ getAdminPanelUrl("/jobs/{$job->id}/extra-description/get-form?type={$extraDescriptionType}") }}"
                    data-title="{{ trans('update.add_job_'.$extraDescriptionType) }}"
            >
                {{ trans('update.add_job_'.$extraDescriptionType) }}
            </button>

        </div>

        @php
            $extraDescriptionValues = $job->extraDescriptions->where('type', $extraDescriptionType);
        @endphp
            ...
            <div class="col-12">
                                    ...
                                    <td>
                                        <div class="btn-group dropdown table-actions position-relative">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <button type="button" class="js-job-contents-action-btn dropdown-item d-flex align-items-center mb-3 py-3 px-0 gap-4"
                                                        data-path="{{ getAdminPanelUrl("/jobs/{$job->id}/extra-description/{$extraDescription->id}/edit") }}"
                                                ...
                                                @include('admin.includes.delete_button',[
                                                    'url' => getAdminPanelUrl("/jobs/{$job->id}/extra-description/{$extraDescription->id}/delete"),
                ...
                @else
                    <div class="d-flex-center flex-column px-32 py-120 text-center">
                        ...
                        <h3 class="font-16 font-weight-bold mt-12">{{ trans("update.job_{$extraDescriptionType}_no_result") }}</h3>
                        <p class="mt-4 font-12 text-gray-500">{!! trans("update.job_{$extraDescriptionType}_no_result_hint") !!}</p>
                    </div>
                @endif
            </div>
        </div>
