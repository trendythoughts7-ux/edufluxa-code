<form id="jobFaqsForm" action="{{ getAdminPanelUrl("/jobs/{$job->id}/faqs/") }}{{ !empty($faq) ? ($faq->id.'/update') : 'store' }}" method="post">

    @if(!empty(getGeneralSettings('content_translate')))
        <div class="form-group">
            <label class="input-label">{{ trans('auth.language') }}</label>
            <select name="locale" class="form-control {{ !empty($faq) ? 'js-job-content-locale' : '' }}" data-title="{{ trans('update.edit_faq') }}">
