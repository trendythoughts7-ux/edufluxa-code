    ...
    <div class="col-12 col-lg-6 mt-20">
        ...
        <div class="p-16 rounded-16 border-gray-200">
            ...
            <x-landingBuilder-select
                label="{{ trans('update.banner_style') }}"
                name="contents[banner_style]"
                value="{{ (!empty($contents) and !empty($contents['banner_style'])) ? $contents['banner_style'] : '' }}"
                :items="['full_width', 'container']"
                hint=""
                className=""
            />
                    ...
                    @foreach($contents['specific_banners'] as $sKey => $itemData)
                        @if($sKey != 'record')
                            <x-landingBuilder-accordion
                                ...
                                show=""
                            >
                                @include('landingBuilder.admin.components.manage.banner_full_width.banner',['itemKey' => $sKey, 'bannerData' => $itemData])
                            </x-landingBuilder-accordion>
                        @endif
                    @endforeach
                @endif
...
<div class="js-banner-item-main-card d-none">
    <x-landingBuilder-accordion
        ...
        show="true"
    >
        @include('landingBuilder.admin.components.manage.banner_full_width.banner')
    </x-landingBuilder-accordion>
</div>
