...
class CartPricingEngineService
{
    ...
    public function productCountBySeller($carts)
    {
        $productCount = [];
    ...
    public function getCouponDiscountByCartItem($couponDiscount, $cart, $user)
    {
        $applyDiscount = false;
    ...
    public function taxIsDifferent($carts)
    {
        $cartHasWebinar = array_filter($carts->pluck('webinar_id')->toArray());
        $cartHasBundle = array_filter($carts->pluck('bundle_id')->toArray());
        $cartHasMeeting = array_filter($carts->pluck('reserve_meeting_id')->toArray());
    ...
    public function handleDiscountPrice($discount, $carts, $subTotal)
    {
        $user = apiAuth();
