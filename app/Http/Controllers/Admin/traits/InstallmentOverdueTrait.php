...
trait InstallmentOverdueTrait
{
    ...
    public function overdueListsExportExcel(Request $request)
    {
        ...
        $exportRecord = Export::create([
            'user_id' => auth()->id(),
            'type' => 'installment_overdue_lists',
            'status' => Export::STATUS_PENDING,
        ]);
        ProcessGenericExport::dispatch(InstallmentOverdueExport::class, $orders, $exportRecord->id, 'overdueLists');
    ...
    public function overdueHistoriesExportExcel(Request $request)
    {
        ...
        $exportRecord = Export::create([
            'user_id' => auth()->id(),
            'type' => 'installment_overdue_histories',
            'status' => Export::STATUS_PENDING,
        ]);
        ProcessGenericExport::dispatch(InstallmentOverdueHistoriesExport::class, $orders, $exportRecord->id, 'overdueHistories');
        return redirect()->back()->with('toast', ['status' => 'success', 'title' => 'Export', 'msg' => 'Your export is processing. You will get a notification when it is ready to download.']);
    }
}
