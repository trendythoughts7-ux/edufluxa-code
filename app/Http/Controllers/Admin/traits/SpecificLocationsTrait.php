...
trait SpecificLocationsTrait
{
        ...
        if (count($items)) {
            foreach ($items as $item) {
                $insert[] = [
                    ...
                    'geo_center' => (!empty($item['latitude']) and !empty($item['longitude'])) ? DB::raw('point(' . (float)$item['latitude'] . ',' . (float)$item['longitude'] . ')') : null,
    ...
    public function updateOrCreateSpecificLocation($targetableId, $targetableType, $data)
    {
        ...
        SpecificLocation::query()->create([
            ...
            'geo_center' => (!empty($data['latitude']) and !empty($data['longitude'])) ? DB::raw('point(' . (float)$data['latitude'] . ',' . (float)$data['longitude'] . ')') : null,
