...
class InstallmentsController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        $ordersCount = InstallmentOrder::query()->where('installment_id', $installment->id)->count();
        if ($ordersCount > 0) {
            $financialFieldsChanged = (
                ($data['target_type'] ?? null) != $installment->target_type ||
                ($data['target'] ?? null) != $installment->target ||
                ($data['capacity'] ?? null) != $installment->capacity ||
                ($data['upfront'] ?? null) != $installment->upfront ||
                ($data['upfront_type'] ?? null) != $installment->upfront_type
            );
            if ($financialFieldsChanged) {
                $toastData = [
                    'title' => trans('public.request_failed'),
                    'msg' => trans('update.installment_financial_fields_locked_has_orders'),
                    'status' => 'error'
                ];
                return redirect(getAdminPanelUrl("/financial/installments/{$installment->id}/edit"))->with(['toast' => $toastData]);
            }
    ...
    public function delete($id)
    {
        ...
        if (!hasValidAdminUrlSignature(getAdminPanelUrl("/financial/installments/{$id}/delete"), request()->query('expires'), request()->query('signature'))) {
            $toastData = [
                'title' => trans('public.request_failed'),
                'msg' => trans('update.installment_delete_link_expired_or_invalid'),
                'status' => 'error'
            ];
        ...
        if ($ordersCount > 0) {
            $toastData = [
                'title' => trans('public.request_failed'),
                'msg' => trans('update.installment_cannot_be_deleted_has_orders'),
                'status' => 'error'
            ];
