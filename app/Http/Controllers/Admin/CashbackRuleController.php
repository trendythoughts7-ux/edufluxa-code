...
class CashbackRuleController extends Controller
{
    ...
    {
        $this->authorize('admin_cashback_rules');
        if (!hasValidAdminUrlSignature(getAdminPanelUrl("/cashback/rules/{$id}/delete"), request()->query('expires'), request()->query('signature'))) {
            $toastData = [
                'title' => trans('public.request_failed'),
                'msg' => trans('update.cashback_rule_delete_link_expired_or_invalid'),
                'status' => 'error'
            ];
