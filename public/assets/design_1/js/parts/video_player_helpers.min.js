...
(function ($) {
  ...
  function ensureWatermarkStylesInjected() {
    ...
    var css = '' + '.moving-watermark{position:absolute;top:0;left:0;z-index:9999;pointer-events:none;opacity:.25;transition:opacity 400ms ease;}' + '.moving-watermark__content{display:flex;align-items:center;gap:8px;background:rgba(0,0,0,.35);color:#fff;border-radius:999px;padding:6px 10px;backdrop-filter:blur(2px);}' + '.moving-watermark__avatar{width:22px;height:22px;border-radius:50%;object-fit:cover;display:block}' + '.moving-watermark__name{font-size:12px;line-height:1;white-space:nowrap;max-width:220px;overflow:hidden;text-overflow:ellipsis;}' + /* Platform logo only mode overrides */
    '.moving-watermark--logo-only .moving-watermark__content{padding:4px 6px;border-radius:8px;background:rgba(0,0,0,.25);}' + '.moving-watermark--logo-only .moving-watermark__avatar{width:auto;height:22px;border-radius:4px;object-fit:contain;}' + '.moving-watermark--logo-only .moving-watermark__name{display:none;}';
  ...
  window.initMovingWatermark = function (target, opts) {
    try {
      ...
      try {
        wmEl.style.transform = 'translate(8px,8px) scale(' + scale + ')';
      } catch (e) {}
      // Add logo-only class for proper rectangular logo rendering
      if (dataType === 'platform_logo') {
        try {
          wmEl.classList.add('moving-watermark--logo-only');
        } catch (e) {}
      }
